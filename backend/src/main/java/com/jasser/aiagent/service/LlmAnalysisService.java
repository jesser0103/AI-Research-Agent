...
public class LlmAnalysisService {
    private static final String SYSTEM_PROMPT= """
            ...
            """;
    private final WebClient antropicWebClient;
    ...
    public TrendAnalysis analyze(final List<ScrapedPost> posts) {
        ...
        for(int attempt=1;attempt<=3;attempt++){
            try{
                //Antropic messages Api: system is a top level field, not inside messages
                ...
                rawResponse=this.antropicWebClient.post()
                        .uri("/messages")
        ...
        if(rawResponse==null){
            log.error("ALl anthropic attempts failed to respond");
            rawResponse="{}";
        }
        TrendAnalysis analysis=TrendAnalysis.builder()
                .rawAnalysis(rawResponse)
                .postAnalysis(posts.size())
                .build();
    ...
    private List<TrendTopic> parseTopics(String rawResponse, TrendAnalysis analysis) {
        ...
        try{
            ...
            if(!contentArray.isArray()||contentArray.isEmpty()){
                log.error("Unexpected Anthropic response structure: {}",rawResponse);
                return topics;
            }
            ...
            for(final JsonNode node:trendsArray){
                try {
                    ...
                    final TrendTopic topic=TrendTopic.builder()
                            ...
                            .trendScore(node.path("trendScore").asInt(0))
                    ...
                    log.info("Failed to parse trend topic {}",node,e);
                }
            }

        }catch( final Exception e){
            log.info("Failed to parse Anthropic response:{}",rawResponse,e);
        }
        return topics;
    }
